...
const HalaqaDetails = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setSessions(
        sessionsRes.data.filter((session) =>
          studentIds.has(session.student_id),
        ),
      );
    } catch (error) {
  ...
  const typeName =
    types.find((type) => type.id === halaqa?.type_id)?.name || "-";

  const markAbsent = async (student) => {
    try {
      await halaqasAPI.markAbsent(id, student.id, {
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("halaqaType")}</p>
