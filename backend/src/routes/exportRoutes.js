...
router.get("/students/excel", requireRoles("admin", "staff"), async (req, res, next) => {
  try {
...
router.get("/students/pdf", requireRoles("admin", "staff"), async (req, res, next) => {
  try {
...
router.get("/report/:studentId/pdf", authenticate, async (req, res, next) => {
  try {
    ...
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename=student_report_${req.params.studentId}.pdf`);
    res.send(buffer);
  } catch (error) {
    next(error);
  }
