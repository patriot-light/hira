...
async function syncLinkedUser(profile, data) {
  ...
  await getCollection("users").updateOne(
    { id: profile.user_id },
    { $set: { full_name: data.full_name, email: data.email || null } },
  );
}
...
async function listStudents(currentUser) {
  if (currentUser.role === "student") {
    return getCollection("students")
      .find({ user_id: currentUser.id }, { projection: { _id: 0 } })
      .toArray();
  }
  ...
  if (currentUser.role === UserRole.EXAM_TEACHER) {
    ...
    const requestByStudent = new Map(
      requests.map((request) => [request.student_id, request]),
    );
    if (!requests.length) return [];
    const students = await getCollection("students")
      .find(
        { id: { $in: requests.map((request) => request.student_id) } },
        { projection: { _id: 0 } },
      )
      .toArray();
    return students.map((student) => ({
      ...student,
      exam_request: requestByStudent.get(student.id),
    }));
  }
  return getCollection("students")
    .find({}, { projection: { _id: 0 } })
...
async function canAccessStudent(currentUser, studentId) {
  ...
  if (currentUser.role === UserRole.STUDENT) {
    const student = await getCollection("students").findOne(
      { id: studentId, user_id: currentUser.id },
      { projection: { _id: 0 } },
    );
    return !!student;
  }
  ...
  if (currentUser.role === UserRole.EXAM_TEACHER) {
    return !!(await getCollection("exam_requests").findOne(
...
async function getStudentProfile(id, currentUser) {
  ...
  if (currentUser.role === UserRole.EXAM_TEACHER) {
    const examRequest = await getCollection("exam_requests").findOne(
      { student_id: id, status: "pending" },
      { projection: { _id: 0 } },
    );
...
async function listStudentAttendance(studentId, currentUser) {
  if (!(await canAccessStudent(currentUser, studentId)))
    throw httpError(403, "Insufficient permissions");
...
async function updateStudentProfile(id, data) {
  const profile = await getCollection("students").findOne(
    { id },
    { projection: { _id: 0 } },
  );
  ...
  update.halaqa_ids =
    update.halaqa_ids || (update.halaqa_id ? [update.halaqa_id] : []);
...
async function deleteProfile(collection, id, notFoundMessage) {
  const profile = await getCollection(collection).findOne(
    { id },
    { projection: { _id: 0 } },
  );
  if (profile?.user_id)
    await getCollection("users").deleteOne({ id: profile.user_id });
  const result = await getCollection(collection).deleteOne({ id });
  if (!result.deletedCount) throw httpError(404, notFoundMessage);
  if (collection === "teachers") {
    const halaqas = await getCollection("halaqas")
      .find({ teacher_ids: id }, { projection: { _id: 0 } })
      .toArray();
    for (const halaqa of halaqas) {
      await getCollection("halaqas").updateOne(
        { id: halaqa.id },
        {
          $set: {
            teacher_ids: (halaqa.teacher_ids || []).filter(
...
async function updateTeacherProfile(id, data) {
  ...
  const update = { ...data };
  delete update.password;
  const result = await getCollection("teachers").updateOne(
    { id },
    { $set: update },
  );
...
async function updateStaffProfile(id, data) {
  ...
  const update = { ...data };
  delete update.password;
  const result = await getCollection("staff").updateOne(
    { id },
    { $set: update },
  );
...
async function updateUserRole(id, role) {
  ...
  if ([UserRole.TEACHER, UserRole.EXAM_TEACHER].includes(role)) {
    const existingTeacher = await getCollection("teachers").findOne(
      { user_id: id },
      { projection: { _id: 0 } },
    );
    if (!existingTeacher) {
...
async function raiseStudentForExam(studentId, data, currentUser) {
  ...
  if (
    ...
    !(await canAccessStudent(currentUser, studentId))
  ) {
    throw httpError(403, "Insufficient permissions");
  }
  const student = await getCollection("students").findOne(
  ...
  if (!student) throw httpError(404, "Student not found");
  await getCollection("exam_requests").updateMany(
    { student_id: studentId, status: "pending" },
