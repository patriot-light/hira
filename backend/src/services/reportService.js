...
async function dashboard() {
  const pageEvaluations = await getCollection("page_evaluations")
    .find({}, { projection: { _id: 0 } })
    .toArray();
  const examEvaluations = await getCollection("exam_evaluations")
    .find({}, { projection: { _id: 0 } })
    .toArray();
  const sessions = await getCollection("recitation_sessions")
    .find({}, { projection: { _id: 0 } })
    .toArray();
  return {
    total_students: await getCollection("students").countDocuments({}),
    active_students: await getCollection("students").countDocuments({
      status: "active",
    }),
    ...
    total_page_evaluations: await getCollection(
      "page_evaluations",
    ).countDocuments({}),
    total_juz_evaluations: await getCollection(
      "juz_evaluations",
    ).countDocuments({}),
    total_exam_evaluations: await getCollection(
      "exam_evaluations",
    ).countDocuments({}),
    average_page_score: round(
      examEvaluations.length
        ? avg(examEvaluations, "final_score")
        : avg(pageEvaluations, "score"),
    ),
...
async function studentReport(studentId) {
  ...
  const pageEvaluations = await getCollection("page_evaluations")
    ...
    .toArray();
  const juzEvaluations = await getCollection("juz_evaluations")
    .find({ student_id: studentId }, { projection: { _id: 0 } })
    .toArray();
  const examEvaluations = await getCollection("exam_evaluations")
    .find({ student_id: studentId }, { projection: { _id: 0 } })
  ...
  for (const session of sessions) {
    for (const error of session.errors || [])
      error_breakdown[error.category || "unknown"] =
        (error_breakdown[error.category || "unknown"] || 0) + 1;
  }
  const completed = new Set(
    juzEvaluations
      .filter((row) => row.memorization_score >= 70 && row.mastery_score >= 70)
      .map((row) => row.juz_number),
  );
  return {
    student,
    ...
    average_session_score: round(avg(sessions, "final_score")),
    total_pages_read: sessions.reduce(
      (sum, row) => sum + Number(row.total_pages || 0),
      0,
    ),
    total_errors: sessions.reduce(
      (sum, row) => sum + Number(row.total_errors || 0),
      0,
    ),
    error_breakdown,
...
async function halaqaReport(halaqaId) {
  ...
  const sessions = await getCollection("recitation_sessions")
    .find({ student_id: { $in: studentIds } }, { projection: { _id: 0 } })
    .toArray();
  const pageEvaluations = await getCollection("page_evaluations")
    .find({ student_id: { $in: studentIds } }, { projection: { _id: 0 } })
    .toArray();
  const student_performance = students
    .map((student) => {
...
async function teacherReport(teacherId) {
  ...
  const halaqas = await getCollection("halaqas")
    .find({ teacher_ids: teacherId }, { projection: { _id: 0 } })
    .toArray();
  const sessions = await getCollection("recitation_sessions")
    .find({ teacher_id: teacherId }, { projection: { _id: 0 } })
    .toArray();
