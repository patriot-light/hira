...
async function assertCanManageHalaqaAttendance(halaqaId, currentUser) {
  ...
  if (currentUser.role !== "teacher") throw httpError(403, "Insufficient permissions");
...
async function markStudentAbsent(halaqaId, studentId, data, currentUser) {
  ...
  const student = await getCollection("students").findOne({ id: studentId }, { projection: { _id: 0 } });
  if (!student) throw httpError(404, "Student not found");
  const studentHalaqas = student.halaqa_ids || (student.halaqa_id ? [student.halaqa_id] : []);
  if (!studentHalaqas.includes(halaqaId)) throw httpError(400, "Student is not assigned to this halaqa");
...
async function listAttendance(halaqaId, currentUser) {
  ...
  return getCollection("attendance_records").find({ halaqa_id: halaqaId }, { projection: { _id: 0 } }).toArray();
}

async function listTypes() {
  return getCollection("halaqa_types").find({ is_active: true }, { projection: { _id: 0 } }).toArray();
}
...
async function updateType(id, data) {
  const result = await getCollection("halaqa_types").updateOne({ id }, { $set: data });
  if (!result.matchedCount) throw httpError(404, "Halaqa type not found");
  return getCollection("halaqa_types").findOne({ id }, { projection: { _id: 0 } });
}

async function deleteType(id) {
  const inUse = await getCollection("halaqas").findOne({ type_id: id }, { projection: { _id: 0 } });
  if (inUse) throw httpError(400, "Halaqa type is used by existing halaqas");
  const result = await getCollection("halaqa_types").updateOne({ id }, { $set: { is_active: false } });
  if (!result.matchedCount) throw httpError(404, "Halaqa type not found");
}
